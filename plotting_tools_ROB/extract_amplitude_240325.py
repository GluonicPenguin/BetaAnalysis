...
def main():
  ...
  labels = ["W5 new (135 V)","W12 15e14 (210 V) G = 4"]
  ...
  for i in range(1):
    ...
    for j in range(num_curves):
      ...
      threshold = 0.1 * np.max(reshaped_ampl_data[j])
      peak_region = np.where(reshaped_ampl_data[j] > threshold)[0]
      ...
      if make_plots:
        ...
        #plt.plot(reshaped_reshaped_time_data[0],reshaped_ampl_data[0],linestyle='-',color=colours[i],linewidth=1.5,alpha=alphas[i],label=labels[i])
  ...
  if make_populated_plots:
    ...
    axes[0].scatter(a_max,a_para,c='r',label=r'Parabola',marker='o',s=20,edgecolors='black')
    axes[0].scatter(a_max,a_gaus,c='g',label=r'Gaussian',marker='D',s=20,edgecolors='black')
    axes[0].set_xlabel(r"A$_{max}$ / mV",fontsize=14)
    axes[0].set_ylabel(r"A$_{fit}$ / mV",fontsize=14)
    axes[0].set_xlim(0, None)
    axes[0].set_ylim(0, None)
    axes[0].legend(fontsize=14)

    axes[1].scatter(para_mae,gaus_mae,c='blue',label=r'Mean absolute error',marker='o',s=20,edgecolors='black')
    axes[1].set_xlabel(r"MAE$_{para}$",fontsize=14)
    axes[1].set_ylabel(r"MAE$_{Gaus}$",fontsize=14)
    axes[1].set_xscale("log")
    axes[1].set_yscale("log")
    axes[1].legend(fontsize=14)

    axes[2].scatter(para_rmse,gaus_rmse,c='blue',label=r'Root mean square error',marker='o',s=20,edgecolors='black')
    axes[2].set_xlabel(r"RMSE$_{para}$",fontsize=14)
    axes[2].set_ylabel(r"RMSE$_{Gaus}$",fontsize=14)
