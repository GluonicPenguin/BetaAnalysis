...
class plotTRVar:
  ...
  def run(self, file, file_index, tree, channel_array, mcp_tr):
    ...
    for i, thisHist in enumerate(hists_to_plot):
      thisFit = plot_fit_curves(-1*self.xUpper, self.xUpper, "gaus", hists_to_plot[i], channel_of_dut[i], arr_of_biases[i])
      thisFit.Draw("SAME")
    ...
    for i, nom_up_down_hists in enumerate(arr_of_hists):
      ...
      for j, toa_thresh_hist in enumerate(nom_up_down_hists):
        thisFit = plot_fit_curves(-1*self.xUpper, self.xUpper, "gaus", toa_thresh_hist, channel_of_dut[i], arr_of_biases[i])
