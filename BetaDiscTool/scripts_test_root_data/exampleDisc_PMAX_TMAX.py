...
num_events = len(pmax)
sig_events = np.count_nonzero(pmax >= 11)
...
class SignalProbabilityModel(nn.Module):
  def __init__(self):
    super().__init__()
    self.amp_mu = nn.Parameter(torch.tensor(30.0))   # Langaus center
    self.amp_sigma = nn.Parameter(torch.tensor(1.0)) # Langaus width
...
for num_epochs in arr_num_epochs:
  ...
  for prob_threshold in arr_prob_threshold:
    ...
    popt_gaussian, _ = curve_fit(gaussian, bin_centres, bin_heights, p0=[1-(sig_events / num_events), 5, 2])
